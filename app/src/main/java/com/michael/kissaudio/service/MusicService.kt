package com.michael.kissaudio.service
...
import com.michael.kissaudio.MainActivity
import com.michael.kissaudio.widget.PlayerWidgetProvider
...
class MusicService : MediaSessionService() {
    ...
    private fun broadcastWidgetUpdate(player: Player) {
        val intent = Intent(this, com.michael.kissaudio.widget.PlayerWidgetProvider::class.java)
        intent.action = com.michael.kissaudio.widget.PlayerWidgetProvider.ACTION_UPDATE_WIDGET
        ...
        serviceScope.launch(Dispatchers.IO) {
            val db = (application as com.michael.kissaudio.KISSAudioApp).database
            val config = db.appConfigDao().getConfigSync() ?: com.michael.kissaudio.data.AppConfig()
            ...
            when (category) {
                "PODCASTS" -> {
                    ...
                    iconRes = com.michael.kissaudio.R.drawable.ic_widget_podcast
                }
                "RADIO" -> {
                    title = metadata.title?.toString() ?: "Radio"
                    subtitle = metadata.artist?.toString() ?: "Live Stream"
                    iconRes = com.michael.kissaudio.R.drawable.ic_widget_radio
                }
                else -> { // MUSIC
                    title = metadata.title?.toString() ?: "Total Audio Hub"
                    subtitle = metadata.artist?.toString() ?: "Unknown Artist"
                    iconRes = com.michael.kissaudio.R.drawable.ic_widget_music
                }
            }
            
            withContext(Dispatchers.Main) {
                intent.putExtra(com.michael.kissaudio.widget.PlayerWidgetProvider.EXTRA_TITLE, title)
                intent.putExtra(com.michael.kissaudio.widget.PlayerWidgetProvider.EXTRA_SUBTITLE, subtitle)
                intent.putExtra(com.michael.kissaudio.widget.PlayerWidgetProvider.EXTRA_IS_PLAYING, isPlaying)
                intent.putExtra(com.michael.kissaudio.widget.PlayerWidgetProvider.EXTRA_POSITION, position)
                intent.putExtra(com.michael.kissaudio.widget.PlayerWidgetProvider.EXTRA_DURATION, duration)
                intent.putExtra(com.michael.kissaudio.widget.PlayerWidgetProvider.EXTRA_CATEGORY, category)
                intent.putExtra(com.michael.kissaudio.widget.PlayerWidgetProvider.EXTRA_ICON_RES, iconRes)
    ...
    private fun handleSkip(forward: Boolean) {
        ...
        serviceScope.launch(Dispatchers.IO) {
            ...
            when (config.lastCategory) {
                ...
                "RADIO" -> {
                    val channels = db.audioChannelDao().getAllChannelsSync().filter { it.type == com.michael.kissaudio.data.ChannelType.RADIO }
                    if (channels.isNotEmpty()) {
                        ...
                        val intent = Intent(this@MusicService, MainActivity::class.java).apply {
                            action = "com.michael.kissaudio.ACTION_LOAD_CHANNEL"
                ...
                "PODCASTS" -> {
                    ...
                    if (episodes.isNotEmpty()) {
                        ...
                        val intent = Intent(this@MusicService, MainActivity::class.java).apply {
                            action = "com.michael.kissaudio.ACTION_LOAD_EPISODE"
